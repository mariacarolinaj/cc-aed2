...
public class TP01Q14PalindromoRecursivo {

    public static boolean isEquals(char primeiro, char segundo) {
        return primeiro == segundo;
    }

    public static boolean isPalindromo(int posicao, String palavra) {
        boolean palindromo = true;
        int posicaoFinal = palavra.length() - 1;

        if (posicao < palavra.length() / 2) {
            if (isEquals(palavra.charAt(posicao), palavra.charAt(posicaoFinal - posicao))) {
                palindromo = palindromo && isPalindromo(posicao + 1, palavra);
            } else {
                palindromo = false;
            }
        }
