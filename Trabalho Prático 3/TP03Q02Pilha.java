        ...
        for (int indiceOperacao = 0; indiceOperacao < quantidadeOperacoes; indiceOperacao++) {
            ...
            if (operacao != "") {
                ...
                case "I":
                    ...
                    times.empilhar(processarArquivo(caminhoTimeInicio));
                    break;
                case "R":
                    MyIO.println("(R) " + times.desempilhar().getNome());
                    break;
                default:
...
class Pilha {
    ...
    public Time desempilhar() throws Exception {
        // verifica se a pilha possui algum item para ser desempilhado
        if (Topo == null) {
            throw new Exception("A pilha está vazia.");
        }

        // armazena o time que foi desempilhado na variável timeDesempilhado para
        // retorna-lo e modifica o topo da pilha
        Time timeDesempilhado = Topo.Elemento;
        Topo = Topo.Proximo;

        return timeDesempilhado;
    }

    public boolean pesquisar(Time time) {
        boolean elementoExiste = false;
        for (Celula celula = Topo; celula.Proximo != null && elementoExiste == false; celula = celula.Proximo) {
    ...
    public int obterTamanho() {
        ...
        for (Celula celula = Topo; celula.Proximo != null; tamanho++, celula = celula.Proximo)
            ;
    ...
    public void imprimir() {
        this.imprimir(Topo, obterTamanho());
    }

    public void imprimir(Celula celula, int indice) {
        // imprime recursivamente os elementos da pilha
        if (celula.Proximo != null) {
            this.imprimir(celula.Proximo, indice - 1);
        }
