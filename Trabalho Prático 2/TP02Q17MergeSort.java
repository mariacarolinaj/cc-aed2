...
public class TP02Q17MergeSort {
    ...
    public static int obtemCapacidadeEstadio(String secao) {
        ...
        capacidade = capacidade.replace("(", "remover");

        if (capacidade.contains("remover")) {
            capacidade = capacidade.split("remover")[0];
        }

        if (capacidade.contains(",")) {
            int quantidadeNumerosDepois = capacidade.split(",")[1].length();
            if (quantidadeNumerosDepois == 2) {
                capacidade += "0";
            } else if (quantidadeNumerosDepois > 3) {
                String[] splitCapacidade = capacidade.split(",");
                capacidade = splitCapacidade[0] + splitCapacidade[1].substring(0, 3);
            }
        }
...
class Lista {
    ...
    public int obtemMaiorCapacidade() {
        int maiorCapacidade = Itens[0].getCapacidade();

        for (int indice = 0; indice < QuantidadeItens; indice++) {
            if (maiorCapacidade < Itens[indice].getCapacidade()) {
                maiorCapacidade = Itens[indice].getCapacidade();
            }
        }
    ...
    public void ordenarPorMergeSort() {
        ordenarPorMergeSort(Itens, 0, QuantidadeItens - 1);
    }

    public void ordenarPorMergeSort(Time itens[], int esquerda, int direita) {
        if (esquerda < direita) {
            // posicao da metade do vetor
            int posicaoCentral = (esquerda + direita) / 2;

            ordenarPorMergeSort(itens, esquerda, posicaoCentral);
            ordenarPorMergeSort(itens, posicaoCentral + 1, direita);

            // realiza merge das duas metades do vetor já ordenadas
            mergeMetadesOrdenadas(itens, esquerda, posicaoCentral, direita);
        }
    }

    private void mergeMetadesOrdenadas(Time itens[], int esquerda, int metade, int direita) {
        int quantidadeItensEsquerda = metade - esquerda + 1;
        int quantidadeItensDireita = direita - metade;

        Time itensEsquerda[] = new Time[quantidadeItensEsquerda];
        Time itensDireita[] = new Time[quantidadeItensDireita];

        // armazena os dados referentes aos itens da esquerda e da direita do vetor
        for (int indice = 0; indice < quantidadeItensEsquerda; indice++)
            itensEsquerda[indice] = itens[esquerda + indice];

        for (int indice = 0; indice < quantidadeItensDireita; indice++)
            itensDireita[indice] = itens[metade + 1 + indice];

        int indiceAuxiliarEsquerda = 0, indiceAuxiliarDireita = 0;

        int indiceAuxiliarVetorOrdenado = esquerda;

        while (indiceAuxiliarEsquerda < quantidadeItensEsquerda && indiceAuxiliarDireita < quantidadeItensDireita) {
            if (desempateAlfabetico(itensEsquerda[indiceAuxiliarEsquerda], itensDireita[indiceAuxiliarDireita])
                    .getNome().equals(itensEsquerda[indiceAuxiliarEsquerda].getNome())) {
                itens[indiceAuxiliarVetorOrdenado] = itensEsquerda[indiceAuxiliarEsquerda];
                indiceAuxiliarEsquerda++;
            } else {
                itens[indiceAuxiliarVetorOrdenado] = itensDireita[indiceAuxiliarDireita];
                indiceAuxiliarDireita++;
            }
            indiceAuxiliarVetorOrdenado++;
        }

        // armazena os valores já ordenados dos subvetores da esquerda e direita no
        // vetor original

        while (indiceAuxiliarEsquerda < quantidadeItensEsquerda) {
            itens[indiceAuxiliarVetorOrdenado] = itensEsquerda[indiceAuxiliarEsquerda];
            indiceAuxiliarEsquerda++;
            indiceAuxiliarVetorOrdenado++;
        }

        while (indiceAuxiliarDireita < quantidadeItensDireita) {
            itens[indiceAuxiliarVetorOrdenado] = itensDireita[indiceAuxiliarDireita];
            indiceAuxiliarDireita++;
            indiceAuxiliarVetorOrdenado++;
        }
    ...
    private Time desempateAlfabetico(Time time1, Time time2) {
        ...
        for (int indice = 0; indice < time1.getNomeArquivo().length()
                && indice < time2.getNomeArquivo().length(); indice++) {
            if ((int) time1.getNomeArquivo().charAt(indice) != (int) time2.getNomeArquivo().charAt(indice)) {
                if ((int) time1.getNomeArquivo().charAt(indice) < (int) time2.getNomeArquivo().charAt(indice)) {
                    menor = time1;
                } else {
                    menor = time2;
                ...
                indice = time1.getNomeArquivo().length();
            }
        }
        return menor;
    }
