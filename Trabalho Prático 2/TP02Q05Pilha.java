...
class Pilha {
    ...
    public Pilha(int quantidadeElementos) {
    ...
    public void empilhar(Time time) throws Exception {
        // verifica se ainda tem espaço para empilhar algum item
        if (QuantidadeItens >= Itens.length) {
            throw new Exception("A pilha está cheia.");
        }

        // insere o time no vetor de itens e incrementa a posição do último item
        // inserido
        Itens[++QuantidadeItens] = time;
    }
    ...
    public Time desempilhar() throws Exception {
        ...
        // retorna-lo e decrementa o ponteiro que armazena o índice do último time
        // inserido na pilha
        Time timeDesempilhado = Itens[QuantidadeItens--];
    ...
    public boolean pesquisar(Time time) {
        ...
        for (int indice = 0; indice < QuantidadeItens && elementoExiste == false; indice++) {
            elementoExiste = (Itens[indice].getNomeArquivo().equals(time.getNomeArquivo()));
        }
