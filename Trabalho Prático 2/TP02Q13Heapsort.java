...
class Lista {
    ...
    private Time desempateAlfabetico(Time time1, Time time2) {
        ...
        for (int indice = 0; indice < time1.getEstadios().length() && indice < time2.getEstadios().length(); indice++) {
            if ((int) time1.getEstadios().charAt(indice) != (int) time2.getEstadios().charAt(indice)) {
                if ((int) time1.getEstadios().charAt(indice) < (int) time2.getEstadios().charAt(indice)) {
                    menor = time1;
                } else {
                    menor = time2;
                ...
                indice = time1.getEstadios().length();
            }
        }
        return menor;
    }
    ...
    public void ordenarPorHeap() {
        Time[] itensAuxiliar = new Time[QuantidadeItens + 1];
        for (int indice = 0; indice < QuantidadeItens; indice++) {
            itensAuxiliar[indice + 1] = Itens[indice];
        }

        Itens = itensAuxiliar;

        // realiza construção do heap
        for (int indice = 2; indice <= QuantidadeItens; indice++) {
            constroi(indice);
        }

        int tamanhoHeap = QuantidadeItens;

        while (tamanhoHeap > 1) {
            swap(1, tamanhoHeap--);
            reconstroi(tamanhoHeap);
        }

        itensAuxiliar = Itens;
        Itens = new Time[QuantidadeItens];
        // ajusta vetor de Times para começar a partir da posição 0 novamente
        for (int i = 0; i < QuantidadeItens; i++) {
            Itens[i] = itensAuxiliar[i + 1];
        }
    }

    public void constroi(int tamanho) {
        for (int indice = tamanho; indice > 1 && desempateAlfabetico(Itens[indice], Itens[indice / 2]).getNome()
                .equals(Itens[indice].getNome()); indice /= 2) {
            swap(indice, indice / 2);
        }
    }

    public void reconstroi(int tamanho) {
        int indiceAuxiliar = 1, indiceFilho;
        while (indiceAuxiliar <= (tamanho / 2)) {

            if (desempateAlfabetico(Itens[2 * indiceAuxiliar], Itens[2 * indiceAuxiliar + 1]).getNome()
                    .equals(Itens[2 * indiceAuxiliar].getNome()) || 2 * indiceAuxiliar == tamanho) {
                indiceFilho = 2 * indiceAuxiliar;
            } else {
                indiceFilho = 2 * indiceAuxiliar + 1;
            }

            if (desempateAlfabetico(Itens[indiceAuxiliar], Itens[indiceFilho]).getNome()
                    .equals(Itens[indiceFilho].getNome())) {
                swap(indiceAuxiliar, indiceFilho);
                indiceAuxiliar = indiceFilho;
            } else {
                indiceAuxiliar = tamanho;
            }
        }
    }

    private void swap(int posicao1, int posicao2) {
        Time auxiliar = Itens[posicao1];

        Itens[posicao1] = Itens[posicao2];
        Itens[posicao2] = auxiliar;
    }
