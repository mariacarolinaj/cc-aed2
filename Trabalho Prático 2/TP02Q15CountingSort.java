...
class Lista {
    ...
    public void ordenarPorCountSort() {
        // o vetor capacidades armazena cada um dos numeros que podem ser combinados até
        // o valor da maior capacidade
        int[] capacidades = new int[obtemMaiorCapacidade() + 1];
        Time[] ordenado = new Time[QuantidadeItens];

        // inicializa o vetor que armazena as ocorrencias das capacidades com 0
        for (int indice = 0; indice < capacidades.length; indice++) {
            capacidades[indice] = 0;
        }

        // conta quantas vezes cada capacidade é encontrada e armazena no item do vetor
        // capacidades correspondente
        for (int indice = 0; indice < QuantidadeItens; indice++) {
            capacidades[Itens[indice].getCapacidade()]++;
        }

        // o vetor capacidades armazena todos os valores menores ou iguais a posicao
        // (capacidade) armazenada
        for (int indice = 1; indice < capacidades.length; indice++) {
            capacidades[indice] += capacidades[indice - 1];
        }

        // armazena os valores ordenados
        for (int indice = QuantidadeItens - 1; indice >= 0; indice--) {
            ordenado[capacidades[Itens[indice].getCapacidade()] - 1] = Itens[indice];
            capacidades[Itens[indice].getCapacidade()]--;
        }

        // transfere o vetor ordenado para o vetor de Itens da lista
        for (int indice = 0; indice < QuantidadeItens; indice++) {
            Itens[indice] = ordenado[indice];
        }
    }
